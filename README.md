...
Если вы не успели обновить подписку до применения ограничений, воспользуйтесь способом получения через Яндекс Переводчик. Этот метод работает даже при белых списках, потому что серверы Яндекса входят в разрешённые подсети.

**Важно:** автоматически обновлять подписку через Яндекс не получится — при добавлении ссылки в клиент параметры конфигов искажаются. Поэтому только **вручную**:
