...
8. 🌐 **Публикует** файлы на зеркала **GitLab** и **Gitea**.
...
- Сразу после сборки файлы автоматически публикуются на **GitLab** и **Gitea** — этот шаг выполняется параллельно и выполняется не более 2 минут.
...
В подписке собраны тысячи конфигов. Большинство VPN‑клиентов при первом добавлении проверяют каждую строку, разбирают параметры и заносят серверы в свою базу. На слабых устройствах или при очень большом списке этот процесс может занять от 30 секунд до пары минут.
...
Мобильная подписка — это случайный набор конфигов, который меняется при каждом обновлении.
Она помогает равномерно распределять нагрузку по серверам и быстрее находить работающие конфиги.

По умолчанию отдаётся 100 случайных конфигов. Чтобы получить другое количество, добавьте ?n= в URL:
https://solovyov-jenya2004.vercel.app/random?n=50 → 50 конфигов
